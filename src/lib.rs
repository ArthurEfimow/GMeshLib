...
impl Texture {
    pub fn create(source : String) -> Texture {
	    ...
	    let meta = CString::new(source).unwrap();

	    let data = stb_image::stb_image::bindgen::stbi_load(meta.as_ptr() as *const i8,&mut width as *mut i32, &mut height as *mut i32, &mut nr_channels as *mut i32, 4);
